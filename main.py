...
def train(train_loader, model, criterion, optimizer, epoch, args, tf_writer):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            ...
            #         tf_writer.add_scalar('grad/' + param['name_list'][ii] + '_' + param['name'], abs(
            #             p.grad.cpu().numpy()).mean(), epoch * len(train_loader) + i)
            #         tf_writer.add_scalar('value/' + param['name_list'][ii] + '_' + param['name'], abs(
            # p.data.cpu().numpy()).mean(), epoch * len(train_loader) + i)
